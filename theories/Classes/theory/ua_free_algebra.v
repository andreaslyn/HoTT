Require Export HoTT.Classes.interfaces.ua_algebraic_theory.
...
Section hom_free_algebra.
  Context `{Funext} {σ : Signature} (C : Carriers σ)
    {I : Type} (e : Equations σ I)
    (A : Algebra σ) `{!IsAlgebraicTheory A e} (f : ∀ s, C s → A s).
...
Section ump_free_algebra.
  Context
    ...
    {I : Type} (e : Equations σ I)
    (A : Algebra σ) `{!IsAlgebraicTheory A e}.
...
Section term_algebra_is_free.
  ...
  Global Instance is_algebraic_equations_term_algebra {σ} (A : Algebra σ)
    : IsAlgebraicTheory A (trivial_equations σ).
  Proof.
    intro e. elim e.
  Defined.
